...
//   const filename = "generated_files/addresses.csv";

//   // what file should we write the merkel proofs too?
//   const output_file = "generated_files/merkleProof.json";

//   //file that has the user claim list
//   const userclaimFile = "generated_files/claimlist.json";
...
const { solidityPack } = require('@ethersproject/abi');
...
fs.createReadStream(filename)
  ...
  .on('end', () => {
    const leaves = results.map(item => keccak256(ethers.utils.solidityPack(['address', 'uint256'], [item.address, item.amount])));
    ...
    const proofs = results.map(item => {
      const leaf = keccak256(ethers.utils.solidityPack(['address', 'uint256'], [item.address, item.amount]));
      return {
    ...
    console.log('Proofs have been saved to proofs.json');
  });

  // Merkle root: 0x5c6d1cbbb6a7f0747692b72e89f7d1474fd7422273f1b640c2a8b5663d09abdb